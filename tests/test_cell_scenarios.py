...
def test_the_cell_is_buildable_at_more_than_one_size() -> None:
    ...
    for knob in (
        ...
        assert knob in defaults, "%s is still a module constant, so no scenario can vary it" % knob
...
def test_a_cup_never_overhangs_the_face_it_seals(size) -> None:
    ...
    assert cup_radius * 2.0 <= size, "a %.0f mm cup on a %.0f mm face seals on air" % (
        cup_radius * 2000.0,
        size * 1000.0,
    )
...
def test_slots_never_overlap_at_any_carton_size(size, rows, cols) -> None:
    ...
    slots = pallet_slots(origin=[0.0, 0.75, 0.1425], box=(size, size, size), rows=rows, cols=cols, layers=1, gap=0.01)
    assert len(slots) == rows * cols
    places = [s["place"] for s in slots]
    for i, a in enumerate(places):
        for b in places[i + 1 :]:
            apart = max(abs(a[0] - b[0]), abs(a[1] - b[1]))
            assert apart >= size - 1e-9, "two %.0f mm cartons are %.0f mm apart, so they intersect" % (
                size * 1000.0,
                apart * 1000.0,
            )
...
def test_every_slot_rests_a_carton_on_the_deck_not_in_it(size) -> None:
    deck_z = 0.1425
    slots = pallet_slots(origin=[0.0, 0.75, deck_z], box=(size, size, size), rows=2, cols=2, layers=1, gap=0.01)
    for slot in slots:
        rest = deck_z + size / 2.0
        assert slot["place"][2] >= rest - 1e-6, "a carton placed at %.4f sits inside a deck at %.4f" % (
            slot["place"][2],
            deck_z,
        )
