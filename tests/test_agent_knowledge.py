...
def _control_source() -> str:
    return (ROOT / "isaac_mcp" / "tools" / "control.py").read_text(
        encoding="utf-8")
...
def test_the_reference_the_agent_is_sent_to_exists() -> None:
    ...
    assert GUIDE.exists(), (
        "run_control tells the agent to read docs/control_library.md")
...
def test_the_guide_names_the_assets_a_cell_is_built_from() -> None:
    ...
    for token in ("conveyorbelt_a05", "/Isaac/People/Characters",
                  "list_props", "find_prop"):
...
def test_the_guide_carries_the_measured_numbers(number, what) -> None:
    ...
    assert number in GUIDE.read_text(encoding="utf-8"), (
        "the guide does not record %s (%s)" % (number, what))
...
def test_every_api_the_docstring_advertises_actually_exists() -> None:
    ...
    for name in ("Scene", "Robot", "Conveyor", "SafetyFence", "list_props",
                 "find_prop", "spawn_prop", "spawn_pedestal",
                 "spawn_operator", "vision"):
        assert hasattr(sim, name), (
            "run_control advertises %s and the package has no such name" % name)
