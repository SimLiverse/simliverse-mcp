...
def _simliverse_note(what):
    # A ScriptNode's own errors go to the Kit console, which nobody reading
    # the controller trace can see. Anything the shim itself gets wrong
    # lands here instead.
    try:
...
def _simliverse_load(node_path, script_path, module_name):
    try:
        spec = _ilu.spec_from_file_location(module_name, script_path)
        module = _ilu.module_from_spec(spec)
        _sys.modules[module_name] = module
        spec.loader.exec_module(module)
        _SIMLIVERSE_NODE_MODULES[node_path] = module
        _simliverse_note("loaded %s for %s" % (script_path, node_path))
    except Exception:
        _simliverse_note("LOAD FAILED for %s: %s" % (node_path, _tb.format_exc()))


_simliverse_load({node_path!r}, {script_path!r}, {module_name!r})
